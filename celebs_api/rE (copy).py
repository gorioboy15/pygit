...
def findNamedef(name): #find it by name if doesnt know the lastname
    morename = []
    ''' Enter a name and it will find : How many have names and the lastnames'''
    ...
    if not result:
        ...
        if yn.lower() == 'yes':
            print()
            return findBday(rslt)
        else:
            return "Sorry it's the only name on file."
...
def findBday(findName): #find birhtday of the celebrity name
    ...
    for k, v in scelbs.items():
        ...
        if findName.lower() == k.lower():  # whatever they input it will like caseignorethe string would be lower
            ...
            if countfindname == 0:
                findNamedef(findName)
            elif countfindname == 1:
                return resltbday

            #reslt = 'Sorry, Nothing in record.'
    return resltbday
